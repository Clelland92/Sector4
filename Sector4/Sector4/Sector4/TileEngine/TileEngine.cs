...
namespace Sector4
{
    ...
    static class TileEngine
    {
        ...
        private static Vector2 UpdateUserMovement(GameTime gameTime)
        {
            ...
            {
                if (CanPartyLeaderMoveUp() && (autoPartyLeaderMovement.X == 0))
                {
                    desiredMovement.Y -= 1;
                    autoPartyLeaderMovement.Y = -(Map.TileSize.Y - 1);
                }
                partyLeaderPosition.Direction = Direction.North;

            }
            if (InputManager.IsActionPressed(InputManager.Action.MoveCharacterDown))
            {
                if (CanPartyLeaderMoveDown() && (autoPartyLeaderMovement.X == 0))
                {
                    desiredMovement.Y += 1;
                    autoPartyLeaderMovement.Y = (Map.TileSize.Y - 1);
                }
            ...
            {
                if (CanPartyLeaderMoveLeft() && (autoPartyLeaderMovement.Y == 0))
                {
                    desiredMovement.X -= 1;
                    autoPartyLeaderMovement.X = -(Map.TileSize.X - 1);
                }
                partyLeaderPosition.Direction = Direction.West;
            }
            if (InputManager.IsActionPressed(InputManager.Action.MoveCharacterRight))
            {
                if (CanPartyLeaderMoveRight() && (autoPartyLeaderMovement.Y == 0))
                {
                    desiredMovement.X += 1;
                    autoPartyLeaderMovement.X = (Map.TileSize.X - 1);
                }
                partyLeaderPosition.Direction = Direction.East;
            }
        ...
        /// </remarks>
        const int movementCollisionTolerance = 0;
